...
if page == "🧮 Calculator":
    ...
    with l:
        ...
        water = c3.number_input("Water Added (mL)", value=2.0, min_value=0.1, format="%.1f")
    ...
    with r:
        ...
        if v_qty > 0 and water > 0 and d_qty > 0:
            ...
            with st.expander(f"📖 Profile: {sel_peptide}", expanded=True):
                ...
                st.markdown(f"**🌟 Key Benefits:**\n{info['benefits_summary']}")
                st.markdown(f'<div class="side-effect-box"><strong>⚠️ Side Effects:</strong><br>{info["side_effects_summary"]}</div>', unsafe_allow_html=True)
    ...
    for idx, (name, i) in enumerate(items.items()):
        with cols[idx % 3]:
            with st.container(border=True):
                ...
                st.markdown(f'<div class="side-effect-box"><strong>⚠️ Side Effects:</strong><br>{i["side_effects_detailed"]}</div>', unsafe_allow_html=True)
