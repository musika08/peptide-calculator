...
# --- PEPTIDE DATA (CALCULATOR VERSION) ---
PEPTIDE_PRESETS = {
    ...
    "Ipamorelin": {"vial_mg": 5.0, "dose_mcg": 100.0, "default_dose_unit": "mcg", "default_stock_unit": "mg", "iu_conversion": None, "type": "Growth Hormone", "benefits_summary": "- Clean GH release\n- Muscle sparing", "side_effects_summary": "- Slight water retention", "protocol_summary": "100-300mcg nightly, fasted."},
    "Kisspeptin": {"vial_mg": 10.0, "dose_mcg": 100.0, "default_dose_unit": "mcg", "default_stock_unit": "mg", "iu_conversion": None, "type": "Hormonal", "benefits_summary": "- Restarts HPTA axis safely\n- Boosts fertility", "side_effects_summary": "- Flushing", "protocol_summary": "100-200mcg daily."},
...
def run_calculator():
    ...
    with left_col:
        ...
        default_index = sorted_presets.index("Tirzepatide") if "Tirzepatide" in sorted_presets else 0
        ...
        syringe_type = st.radio("Syringe Type", ["U-100", "U-40"], horizontal=True)
    ...
    with right_col:
        ...
        if vial_qty > 0 and water_ml > 0 and desired_dose > 0:
            ...
            if conversion and conversion > 1:
                t_stock = (vial_qty if vial_unit == 'mg' else vial_qty/1000) * conversion if vial_unit != 'IU' else vial_qty
                t_target = desired_dose if dose_unit == 'IU' else (desired_dose * conversion if dose_unit == 'mg' else (desired_dose/1000)*conversion)
            elif conversion == 1:
                t_stock, t_target = vial_qty, desired_dose
            else:
                t_stock = vial_qty * 1000 if vial_unit == 'mg' else vial_qty if vial_unit == 'mcg' else vial_qty * 1000000
                t_target = desired_dose * 1000 if dose_unit == 'mg' else desired_dose if dose_unit == 'mcg' else desired_dose * 1000000

            if t_stock > 0:
                draw_ml = t_target / (t_stock / water_ml)
                units = draw_ml * syringe_factor
                d_per_v = t_stock / t_target
            else: draw_ml, units, d_per_v = 0, 0, 0

            c1, c2, c3 = st.columns(3)
            c1.metric("Draw (mL)", f"{draw_ml:.4f}")
    ...
    st.divider()
    with st.expander("🛠️ How to Reconstitute", expanded=True):
        st.markdown(f"1. **Clean:** Wipe vial tops.\n2. **Withdraw:** Draw water.\n3. **Inject:** Slowly into peptide vial.\n4. **Mix:** Swirl gently.\n5. **Store:** Refrigerate.")
